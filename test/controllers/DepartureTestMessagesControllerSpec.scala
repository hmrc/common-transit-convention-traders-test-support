...
class DepartureTestMessagesControllerSpec
  ...
  "DepartureTestMessagesController" - {
    ...
    "POST /movements/departures/:departureId/messages/:messageId" - {
      "must send a test message to the departures backend and return Created if successful" in {
        val mockDepartureConnector        = mock[DepartureConnector]
        val mockInboundRouterConnector    = mock[InboundRouterConnector]
        val mockDepartureMessageConnector = mock[DepartureMessageConnector]
        ...
        running(application) {
          val request =
            FakeRequest(POST, routing.routes.DeparturesRouter.injectEISResponseWithTriggerId(departureString, messageId.value).url).withJsonBody(exampleRequest)
      ...
      "must return UnsupportedMediaType when no Content-Type specified" in {
        ...
        running(application) {
          val request = FakeRequest(
            method = POST,
            uri = routing.routes.DeparturesRouter.injectEISResponseWithTriggerId(departureString, messageId.value).url,
            headers = FakeHeaders(Nil),
            body = AnyContentAsEmpty
          )
      ...
      "must return NotImplemented if specified message type is not supported" in {
        ...
        running(application) {
          val request =
            FakeRequest(POST, routing.routes.DeparturesRouter.injectEISResponseWithTriggerId(departureString, messageId.value).url).withJsonBody(invalidRequest)
