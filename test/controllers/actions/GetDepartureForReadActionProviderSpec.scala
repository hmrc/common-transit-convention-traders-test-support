...
class GetDepartureForReadActionProviderSpec
    extends AnyFreeSpec
    with Matchers
    with MockitoSugar
  ...
  class Harness(get: GetDepartureForReadActionProvider) {

    def get(departureId: DepartureId): Action[AnyContent] = get.apply(departureId) {
      request =>
        Results.Ok(request.departure.departureId.toString)
    }
  }

  "get departure for read" - {

    val eoriNumber = "123"

    "must get a departure when it exists" in {
      ...
      val actionProvider = application.injector().instanceOf[GetDepartureForReadActionProvider]
