...
class DepartureConnector @Inject()(http: HttpClient, appConfig: AppConfig) extends ItemConnector(http, appConfig) {
  ...
  def post(messageType: String, message: String, departureId: DepartureId)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[HttpResponse] =
    super.post(messageType, message, departureId)
