...
object XMLTransformer {
  ...
  def populateMesRecMES6(message: NodeSeq, messageType: String, messages: Seq[MovementMessage]): NodeSeq =
    messages.filter(movementMessage => movementMessage.messageType == messageType).lastOption match {
      case Some(movementMessage: MovementMessage) =>
        updateMessage(message, (movementMessage.message \\ "MesSenMES3").text)
      case None =>
        message
    }
  ...
  private def updateMessage(message: NodeSeq, value: String): NodeSeq = {
    val rewriteRule = new RewriteRule {
      override def transform(n: Node): NodeSeq = n match {
        case e: Elem if e.label.contains("MesRecMES6") => e.copy(child = Text(value))
        case other                                     => other
      }
    }
