...
class UnloadingPermissionGenerator @Inject() (
  ...
  def generate(instructions: GenInstructions): PartialFunction[MessageType, NodeSeq] = {
    case UnloadingPermission => generateUnloadingPermission(instructions.asInstanceOf[UnloadingPermissionGenInstructions])
  }

  private def generateUnloadingPermission(instructions: UnloadingPermissionGenInstructions): NodeSeq =
    <CC043A>
